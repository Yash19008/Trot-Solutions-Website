...
export default function BlogFormClientComponent({ initialBlog }: { initialBlog?: Blog | null }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(error);
      alert("Error saving blog. Ensure slug is unique.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
        <h1 style={{ fontSize: "24px", fontWeight: "bold" }}>{initialBlog ? "Edit Blog" : "New Blog"}</h1>
        <Link href="/admin/blogs" style={{ padding: "8px 16px", backgroundColor: "#f3f4f6", color: "#374151", borderRadius: "4px", textDecoration: "none", fontWeight: "bold" }}>
          Back to List
        </Link>
      </div>
